...
from fx_latlon import *
import time
start = time.time()
### K MEANS FOR LATITUDE - LONGITUDE COORDINATES

if __name__ == "__main__":
    if len(sys.argv) != 5:
        print >> sys.stderr, \
            "Usage: kmeans_latlon.py <k> <euclidean / greatcircle> <input path> <output path>"
        exit(-1)

    sconf = SparkConf() \
        .setAppName("KMeans Spark for Latitude - Longitude Coordinate ") \
    ...
    points =  sc.textFile(file) \
    	...
    	.map(lambda line: [latlon(line[0], line[1]), line[2:]]) \
        .cache()
    ...
    while (sumDist > convergeDist):
        ...
        # 3D (lat-long) sum -- theta(dn)
        clusterSum = closestIndex \
            .map(lambda (index, point): (index, llXYZ(point[0]))) \
    ...
    for index, centroid in centroids.items():
        print "Centroid #" + str(index + 1) + " at," \
            + str(centroid.lat) + "," + str(centroid.lon)
    ...
    output = closestIndex \
        .map(lambda (index, point): \
        str(centroids[index].lat) + "," + str(centroids[index].lon) \
            + "," + str(point[0].lat) + "," + str(point[0].lon) \
